...
def collect_pngs(eye_id=0):
    ...
    args = argparse.ArgumentParser(
        description="Starts raw video stream and collects pngs from all or only selected eyes")

    # adding optional arguments
    aid = args.add_argument("--eye_id", default=None,
                            help="ID (int) of the eye (nano board) to start stream on accordin to"
                                 "settings.network")
...
def main(eye_id=0):
    ...
    # adding optional arguments
    aid = args.add_argument("--eye_id", default=None,
                            help="ID (int) of the eye (nano board) to start eyeserver on as in"
                                 "settings.network")
    args = args.parse_args()
    if args.eye_id is not None:
        eye_ids = [eye['expected_id'] for eye in network.eyes.values()]
...
def start_database():
    ...
    if len(os.listdir(database.database_input_path)) != 0:
        ...
        with_wipe_input = input(
            "Do you want to wipe the input folder and only record data from now on? (y/n)").lower() == 'y'
    else:
...
def thymio_remote_control(thymio_id=0):
    ...
    # adding optional arguments
    aid = args.add_argument("--thymio_id", default=None,
                            help="ID (int) of the thymio (nano board) to start thymioserver on as in"
                                 "settings.network")
    args = args.parse_args()
...
def thymio_autopilot():
    """Starts autopilot for all thymio robots defined in the settings.network"""

    # creating a set of queues (one for each thymio) with center of mass of the fish school
    logger.info("Creating center of mass queue(s)...")
    com_queues = []
    for thymio in network.thymios.values():
        com_queue = Queue()
        com_queues.append(com_queue)

    # creating a set of queues (one for each thymio) with center of mass of the fish school
    logger.info("Creating thymio queue(s)...")
    thymio_queues = []
    for thymio in network.thymios.values():
        thymio_queue = Queue()
        thymio_queues.append(thymio_queue)

    # start database process that read json files written db and forwards them to the thymio
    logger.info(f"Starting database daemon consuming {database.database_input_path}...")
    ...
    # We always wipe the database input folder first (fresh start)
    with_wipe_input = True

    # create the database process that also pushes information into the defined queues for the thymios
    db_process = Process(target=database_daemon_process, args=(database.database_input_path, with_wipe_input,
                                                               com_queues, thymio_queues,))

    # create a set of thymio masters, these are bridge classes between this computer and the thymios via Pyro5
    thmyio_masters = [CoBeThymioMaster(target_thymio_name=f"thymio_{i}") for i in range(len(network.thymios))]

    # create a single process thread for each thymio
    autopilot_processes = []
    for i in range(len(network.thymios)):
        autopilot_process = Process(target=thmyio_masters[i].thymio_autopilot,
                                    args=(thymio_queues[i], com_queues[i], False,))
        autopilot_processes.append(autopilot_process)

    # starting the database daemon
    db_process.start()
    logger.info("Database daemon started and passed com and thymio queues.")

    # starting the autopilot processes
    for i in range(len(network.thymios)):
        autopilot_processes[i].start()
        logger.info(f"Autopilot started on thymio {i}.")

    # wait for user input to stop the autopilot
    input("Press enter to stop autopilot on all robots...")

    # stopping the autopilot processes and request thymios to be stopped
    logger.info("Stopping autopilot...")
    for i in range(len(network.thymios)):
        autopilot_processes[i].terminate()
        autopilot_processes[i].join()
        logger.info("Autopilot stopped on thymio {i}.")

        logger.info(f"Stopping thymio {i}...")
        thmyio_masters[i].stop_thymios()

    # stopping the database daemon
    logger.info("Stopping database daemon...")
    db_process.terminate()
    db_process.join()

    logger.info("Database daemon stopped.")
    logger.info("Autopilot stopped. Bye bye!")
...
def start_thymioserver(th_id=None):
    ...
    # adding optional arguments
    aid = args.add_argument("--thymio_id", default=None,
                            help="ID (int) of the thymio (rpi board) to start thymioserver on as in"
